@section('stylesheets')
	@parent
	{{ HTML::style('packages/antoniputra/asmoyo/admin/plugin/froala_editor/css/froala_editor.min.css') }}
@stop

@section('javascripts')
	@parent
	{{ HTML::script('packages/antoniputra/asmoyo/admin/plugin/froala_editor/js/froala_editor.min.js') }}
	...
	<script>
		var froala_defaults = {
			...
			imageUploadURL: "{{admin_route('media.postFroala')}}",
			imagesLoadURL: "{{admin_route('media.getFroala')}}",
			height: 400,
			inlineMode: false,
			customButtons: {
	            insertCode: {
